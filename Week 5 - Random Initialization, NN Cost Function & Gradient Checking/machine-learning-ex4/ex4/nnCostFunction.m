...
for i = 1:m
  ...
  a1 = x_i'; %converting the row vector into a column vector, x_i(401x1)
  z2 = Theta1 * a1; %Theta1(25x401) * a1(401x1) = z2(25x1)
  a2 = sigmoid(z2);
  %1st Hidden Layer
  a2 = [1; a2]; %a2(26x1)
  ...
  h_i = a3;
  %Output Layer
  y_i = zeros(num_labels, 1); %y_i(10x1)
  pres_label = y(i);
  y_i(pres_label) = 1;
  y_i_temp = y_i'; %y_i_temp(1x10)
  cost_i = (y_i_temp * log(h_i)) + ((1-y_i_temp) * log(1 - h_i));
  J = J + cost_i;
  
  %----------------------BackPropagation----------------------------
  delta_3 = h_i - y_i; %delta3(10x1)
  delta_2 = (Theta2' * delta_3) .* sigmoidGradient([1; z2]); %delta2(26x1)
  
  % ---------Gradients--------
  Theta2_grad = Theta2_grad + delta_3 * a2'; %Theta2_grad(10x26)
  delta_2_temp = delta_2(2:end);
  Theta1_grad = Theta1_grad + delta_2_temp * a1';
endfor
J = (-1/m) * J;
Theta1_grad = (1/m) * Theta1_grad;
Theta2_grad = (1/m) * Theta2_grad;

%--------------------Regularization of Cost Function : Forward Pass-------------------
...
%---------------Regularization of Gradient Matrices: BackPropagation-----------------
Theta1_temp = Theta1(:, 2:end); %removing the 1st column from Theta1, ignoring bias
Theta1_temp = [zeros(size(Theta1, 1), 1) Theta1_temp];
Theta2_temp = Theta2(:, 2:end); %removing the 1st column from Theta2, ignoring bias
Theta2_temp = [zeros(size(Theta2, 1), 1) Theta2_temp];
